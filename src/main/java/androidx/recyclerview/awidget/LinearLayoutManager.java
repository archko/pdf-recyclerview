...
/**
 * A {@link ARecyclerView.LayoutManager} implementation which provides
 * similar functionality to {@link android.widget.ListView}.
 */
public class LinearLayoutManager extends ARecyclerView.LayoutManager implements
        ItemTouchHelper.ViewDropHandler, ARecyclerView.SmoothScroller.ScrollVectorProvider {

    private static final String TAG = "LinearLayoutManager";

    static final boolean DEBUG = false;

    public static final int HORIZONTAL = ARecyclerView.HORIZONTAL;

    public static final int VERTICAL = ARecyclerView.VERTICAL;
     ...
     */
    @ARecyclerView.Orientation
    int mOrientation = ARecyclerView.DEFAULT_ORIENTATION;
     ...
     * {@link #onLayoutChildren(ARecyclerView.Recycler, ARecyclerView.State)} is run.
     */
     ...
     */
    int mPendingScrollPosition = ARecyclerView.NO_POSITION;
    ...
    public LinearLayoutManager(Context context) {
        this(context, ARecyclerView.DEFAULT_ORIENTATION, false);
    }
     ...
     */
    public LinearLayoutManager(Context context, @ARecyclerView.Orientation int orientation,
    ...
    /**
     * Constructor used when layout manager is set in XML by ARecyclerView attribute
     ...
     */
    @Override
    public ARecyclerView.LayoutParams generateDefaultLayoutParams() {
        return new ARecyclerView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
     ...
     * ARecyclerView.
     * <p>
     * If you are using a {@link ARecyclerView.RecycledViewPool}, it might be a good idea to set
     * this flag to <code>true</code> so that views will be available to other RecyclerViews
     * immediately.
     * <p>
     * Note that, setting this flag will result in a performance drop if ARecyclerView
     * is restored.
     *
    ...
    @Override
    public void onDetachedFromWindow(ARecyclerView view, ARecyclerView.Recycler recycler) {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof SavedState) {
            ...
            if (mPendingScrollPosition != ARecyclerView.NO_POSITION) {
     ...
     */
    public void setOrientation(@ARecyclerView.Orientation int orientation) {
     ...
     * When set to true, If {@link ARecyclerView} is LTR, than it will
     * layout from RTL, if {@link ARecyclerView}} is RTL, it will layout
     * from LTR.
     * <p>
     ...
     * @deprecated Use {@link #calculateExtraLayoutSpace(ARecyclerView.State, int[])} instead.
     */
    @SuppressWarnings("DeprecatedIsStillUsed")
    @Deprecated
    protected int getExtraLayoutSpace(ARecyclerView.State state) {
     ...
     * {@link ARecyclerView.State#hasTargetScrollPosition()} to find out if a smooth scroll to a
     * position is in progress, and {@link ARecyclerView.State#getTargetScrollPosition()} to find out
     ...
     */
    protected void calculateExtraLayoutSpace(@NonNull ARecyclerView.State state,
    ...
    @Override
    public void smoothScrollToPosition(ARecyclerView recyclerView, ARecyclerView.State state,
                                       int position) {
     ...
     */
    @Override
    public void onLayoutChildren(ARecyclerView.Recycler recycler, ARecyclerView.State state) {
        // layout algorithm:
        ...
        if (mPendingSavedState != null || mPendingScrollPosition != ARecyclerView.NO_POSITION) {
        ...
        if (!mAnchorInfo.mValid || mPendingScrollPosition != ARecyclerView.NO_POSITION
        ...
        if (state.isPreLayout() && mPendingScrollPosition != ARecyclerView.NO_POSITION
    ...
    @Override
    public void onLayoutCompleted(ARecyclerView.State state) {
        super.onLayoutCompleted(state);
        mPendingSavedState = null; // we don't need this anymore
        mPendingScrollPosition = ARecyclerView.NO_POSITION;
        mPendingScrollPositionOffset = INVALID_OFFSET;
        mAnchorInfo.reset();
    }
     ...
     *                                 indices.
     */
    void onAnchorReady(ARecyclerView.Recycler recycler, ARecyclerView.State state,
     ...
     */
    private void layoutForPredictiveAnimations(ARecyclerView.Recycler recycler,
                                               ARecyclerView.State state, int startOffset,
                                               int endOffset) {
        ...
        final List<ARecyclerView.ViewHolder> scrapList = recycler.getScrapList();
        final int scrapSize = scrapList.size();
        final int firstChildPos = getPosition(getChildAt(0));
        for (int i = 0; i < scrapSize; i++) {
            ARecyclerView.ViewHolder scrap = scrapList.get(i);
            if (scrap.isRemoved()) {
                continue;
            }
    ...
    private void updateAnchorInfoForLayout(ARecyclerView.Recycler recycler, ARecyclerView.State state,
     ...
     */
    private boolean updateAnchorFromChildren(ARecyclerView.Recycler recycler,
                                             ARecyclerView.State state, AnchorInfo anchorInfo) {
        if (getChildCount() == 0) {
            return false;
        }
        final View focused = getFocusedChild();
     ...
     */
    private boolean updateAnchorFromPendingData(ARecyclerView.State state, AnchorInfo anchorInfo) {
        if (state.isPreLayout() || mPendingScrollPosition == ARecyclerView.NO_POSITION) {
            return false;
        }
     ...
     */
    private int fixLayoutEndGap(int endOffset, ARecyclerView.Recycler recycler,
                                ARecyclerView.State state, boolean canOffsetChildren) {
     ...
     * @return The final offset amount for children
     */
    private int fixLayoutStartGap(int startOffset, ARecyclerView.Recycler recycler,
                                  ARecyclerView.State state, boolean canOffsetChildren) {
    ...
    /**
     * <p>Scroll the ARecyclerView to make the position visible.</p>
     *
     * <p>ARecyclerView will scroll the minimum amount that is necessary to make the
     ...
     * <code>item[10]</code>'s bottom is 20 pixels above the ARecyclerView's bottom.
     * <p>
     ...
     *                 start edge of the ARecyclerView.
     * @see #setReverseLayout(boolean)
     ...
     */
    @Override
    public int scrollHorizontallyBy(int dx, ARecyclerView.Recycler recycler,
                                    ARecyclerView.State state) {
     ...
     */
    @Override
    public int scrollVerticallyBy(int dy, ARecyclerView.Recycler recycler,
                                  ARecyclerView.State state) {
        if (mOrientation == HORIZONTAL) {
            return 0;
        }
    ...
    @Override
    public int computeHorizontalScrollOffset(ARecyclerView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeVerticalScrollOffset(ARecyclerView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeHorizontalScrollExtent(ARecyclerView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeVerticalScrollExtent(ARecyclerView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeHorizontalScrollRange(ARecyclerView.State state) {
        return computeScrollRange(state);
    }

    @Override
    public int computeVerticalScrollRange(ARecyclerView.State state) {
        return computeScrollRange(state);
    }

    private int computeScrollOffset(ARecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollExtent(ARecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollRange(ARecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
                                   boolean canUseExistingSpace, ARecyclerView.State state) {
    ...
    void collectPrefetchPositionsForLayoutState(ARecyclerView.State state, LayoutState layoutState,
     ...
     * how many inner items should be prefetched when this LayoutManager's ARecyclerView
     * is nested inside another ARecyclerView.
     *
     * <p>Set this value to the number of items this inner LayoutManager will display when it is
     * first scrolled into the viewport. ARecyclerView will attempt to prefetch that number of items
     * so they are ready, avoiding jank as the inner ARecyclerView is scrolled into the viewport.</p>
     *
     * <p>For example, take a vertically scrolling ARecyclerView with horizontally scrolling inner
     * RecyclerViews. The rows always have 4 items visible in them (or 5 if not aligned). Passing
     * <code>4</code> to this method for each inner ARecyclerView's LinearLayoutManager will enable
     * ARecyclerView's prefetching feature to do create/bind work for 4 views within a row early,
     * before it is scrolled on screen, instead of just the default 2.</p>
     *
     * <p>Calling this method does nothing unless the LayoutManager is in a ARecyclerView
     * nested in another ARecyclerView.</p>
     *
    ...
    @Override
    public void collectAdjacentPrefetchPositions(int dx, int dy, ARecyclerView.State state,
                                                 LayoutPrefetchRegistry layoutPrefetchRegistry) {
    ...
    int scrollBy(int delta, ARecyclerView.Recycler recycler, ARecyclerView.State state) {
        if (getChildCount() == 0 || delta == 0) {
            return 0;
        }
        ensureLayoutState();
     ...
     */
    private void recycleChildren(ARecyclerView.Recycler recycler, int startIndex, int endIndex) {
     ...
     *
     * @param recycler        Recycler instance of {@link ARecyclerView}
     ...
     */
    private void recycleViewsFromStart(ARecyclerView.Recycler recycler, int scrollingOffset,
                                       int noRecycleSpace) {
     ...
     *                        #calculateExtraLayoutSpace}.
     */
    private void recycleViewsFromEnd(ARecyclerView.Recycler recycler, int scrollingOffset,
                                     int noRecycleSpace) {
        final int childCount = getChildCount();
        if (scrollingOffset < 0) {
            if (DEBUG) {
     ...
     *
     * @param recycler    Current recycler that is attached to ARecyclerView
     * @param layoutState Current layout state. Right now, this object does not change but
     *                    we may consider moving it out of this view so passing around as a
     *                    parameter for now, rather than accessing {@link #mLayoutState}
     * @see #recycleViewsFromStart(ARecyclerView.Recycler, int, int)
     * @see #recycleViewsFromEnd(ARecyclerView.Recycler, int, int)
     * @see LayoutState#mLayoutDirection
     */
    private void recycleByLayoutState(ARecyclerView.Recycler recycler, LayoutState layoutState) {
     ...
     * @param state           Context passed by the ARecyclerView to control scroll steps.
     * @param stopOnFocusable If true, filling stops in the first focusable new child
     * @return Number of pixels that it added. Useful for scroll functions.
     */
    int fill(ARecyclerView.Recycler recycler, LayoutState layoutState,
             ARecyclerView.State state, boolean stopOnFocusable) {
        ...
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
            ...
            if (ARecyclerView.VERBOSE_TRACING) {
    ...
    void layoutChunk(ARecyclerView.Recycler recycler, ARecyclerView.State state,
        ...
        ARecyclerView.LayoutParams params = (ARecyclerView.LayoutParams) view.getLayoutParams();
     ...
     */
    View findReferenceChild(ARecyclerView.Recycler recycler, ARecyclerView.State state,
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                if (((ARecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
     ...
     * not in the layout.
     * <p>
     * If ARecyclerView has item decorators, they will be considered in calculations as well.
     * <p>
     * LayoutManager may pre-cache some views that are not necessarily visible. Those views
     * are ignored in this method.
     *
     * @return The adapter position of the first visible item or {@link ARecyclerView#NO_POSITION} if
    ...
    public int findFirstVisibleItemPosition() {
        ...
        return child == null ? ARecyclerView.NO_POSITION : getPosition(child);
    }
     ...
     * {@link ARecyclerView#NO_POSITION} if there aren't any visible items.
     ...
     * are ignored in this method.
     *
     * @return The adapter position of the last visible view or {@link ARecyclerView#NO_POSITION} if
     * there aren't any visible items.
     * @see #findLastCompletelyVisibleItemPosition()
     * @see #findFirstVisibleItemPosition()
     */
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
                                    ARecyclerView.Recycler recycler, ARecyclerView.State state) {
        resolveShouldLayoutReverse();
        if (getChildCount() == 0) {
            return null;
        }
    ...
    static class LayoutState {
        ...
        /**
         * Contains the {@link #calculateExtraLayoutSpace(ARecyclerView.State, int[])}  extra layout
        ...
        /**
         * Equal to {@link ARecyclerView.State#isPreLayout()}. When consuming scrap, if this value
        ...
        /**
         * The most recent {@link #scrollBy(int, ARecyclerView.Recycler, ARecyclerView.State)}
         * amount.
         */
         ...
         */
        List<ARecyclerView.ViewHolder> mScrapList = null;
         ...
         */
        boolean hasMore(ARecyclerView.State state) {
         ...
         */
        View next(ARecyclerView.Recycler recycler) {
            if (mScrapList != null) {
        ...
        private View nextViewFromScrapList() {
            ...
            for (int i = 0; i < size; i++) {
                ...
                final ARecyclerView.LayoutParams lp = (ARecyclerView.LayoutParams) view.getLayoutParams();
                if (lp.isItemRemoved()) {
                    continue;
                }
        ...
        public void assignPositionFromScrapList(View ignore) {
            ...
            if (closest == null) {
                mCurrentPosition = ARecyclerView.NO_POSITION;
            } else {
                mCurrentPosition = ((ARecyclerView.LayoutParams) closest.getLayoutParams())
    ...
    public static class SavedState implements Parcelable {
        ...
        void invalidateAnchor() {
            mAnchorPosition = ARecyclerView.NO_POSITION;
        }
    ...
    static class AnchorInfo {
        ...
        void reset() {
            mPosition = ARecyclerView.NO_POSITION;
        ...
        /**
         * assigns anchor coordinate from the ARecyclerView's padding depending on current
         * layoutFromEnd value
         */
        ...
        boolean isViewValidAsAnchor(View child, ARecyclerView.State state) {
            ARecyclerView.LayoutParams lp = (ARecyclerView.LayoutParams) child.getLayoutParams();
