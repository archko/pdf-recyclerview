...
import static androidx.recyclerview.awidget.ViewInfoStore.InfoRecord.FLAG_APPEAR;
import static androidx.recyclerview.awidget.ViewInfoStore.InfoRecord.FLAG_APPEAR_AND_DISAPPEAR;
import static androidx.recyclerview.awidget.ViewInfoStore.InfoRecord.FLAG_APPEAR_PRE_AND_POST;
import static androidx.recyclerview.awidget.ViewInfoStore.InfoRecord.FLAG_DISAPPEARED;
import static androidx.recyclerview.awidget.ViewInfoStore.InfoRecord.FLAG_POST;
import static androidx.recyclerview.awidget.ViewInfoStore.InfoRecord.FLAG_PRE;
import static androidx.recyclerview.awidget.ViewInfoStore.InfoRecord.FLAG_PRE_AND_POST;
...
class ViewInfoStore {
     ...
     */
    @VisibleForTesting
    final SimpleArrayMap<ARecyclerView.ViewHolder, InfoRecord> mLayoutHolderMap =
            new SimpleArrayMap<>();

    @VisibleForTesting
    final LongSparseArray<ARecyclerView.ViewHolder> mOldChangedHolders = new LongSparseArray<>();
     ...
     */
    void addToPreLayout(ARecyclerView.ViewHolder holder, ARecyclerView.ItemAnimator.ItemHolderInfo info) {
    ...
    boolean isDisappearing(ARecyclerView.ViewHolder holder) {
     ...
     */
    @Nullable
    ARecyclerView.ItemAnimator.ItemHolderInfo popFromPreLayout(ARecyclerView.ViewHolder vh) {
     ...
     */
    @Nullable
    ARecyclerView.ItemAnimator.ItemHolderInfo popFromPostLayout(ARecyclerView.ViewHolder vh) {
        return popFromLayoutStep(vh, FLAG_POST);
    }

    private ARecyclerView.ItemAnimator.ItemHolderInfo popFromLayoutStep(ARecyclerView.ViewHolder vh, int flag) {
        ...
        if (record != null && (record.flags & flag) != 0) {
            record.flags &= ~flag;
            final ARecyclerView.ItemAnimator.ItemHolderInfo info;
            if (flag == FLAG_PRE) {
                info = record.preInfo;
     ...
     */
    void addToOldChangeHolders(long key, ARecyclerView.ViewHolder holder) {
     ...
     * @param info   The information to save
     */
    void addToAppearedInPreLayoutHolders(ARecyclerView.ViewHolder holder, ARecyclerView.ItemAnimator.ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    boolean isInPreLayout(ARecyclerView.ViewHolder viewHolder) {
     ...
     */
    ARecyclerView.ViewHolder getFromOldChangeHolders(long key) {
     ...
     * @param info   The information to save
     */
    void addToPostLayout(ARecyclerView.ViewHolder holder, ARecyclerView.ItemAnimator.ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    void addToDisappearedInLayout(ARecyclerView.ViewHolder holder) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    void removeFromDisappearedInLayout(ARecyclerView.ViewHolder holder) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            return;
        }
    ...
    void process(ProcessCallback callback) {
        for (int index = mLayoutHolderMap.size() - 1; index >= 0; index--) {
            final ARecyclerView.ViewHolder viewHolder = mLayoutHolderMap.keyAt(index);
            ...
            if ((record.flags & FLAG_APPEAR_AND_DISAPPEAR) == FLAG_APPEAR_AND_DISAPPEAR) {
                ...
                // Scrap view. ARecyclerView will handle removing/recycling this.
            } else if (DEBUG) {
     ...
     */
    void removeViewHolder(ARecyclerView.ViewHolder holder) {
    ...
    public void onViewDetached(ARecyclerView.ViewHolder viewHolder) {
        removeFromDisappearedInLayout(viewHolder);
    }

    interface ProcessCallback {
        void processDisappeared(ARecyclerView.ViewHolder viewHolder, @NonNull ARecyclerView.ItemAnimator.ItemHolderInfo preInfo,
                                @Nullable ARecyclerView.ItemAnimator.ItemHolderInfo postInfo);

        void processAppeared(ARecyclerView.ViewHolder viewHolder, @Nullable ARecyclerView.ItemAnimator.ItemHolderInfo preInfo,
                             ARecyclerView.ItemAnimator.ItemHolderInfo postInfo);

        void processPersistent(ARecyclerView.ViewHolder viewHolder, @NonNull ARecyclerView.ItemAnimator.ItemHolderInfo preInfo,
                               @NonNull ARecyclerView.ItemAnimator.ItemHolderInfo postInfo);

        void unused(ARecyclerView.ViewHolder holder);
    }
    ...
    static class InfoRecord {
        ...
        int flags;
        @Nullable
        ARecyclerView.ItemAnimator.ItemHolderInfo preInfo;
        @Nullable
        ARecyclerView.ItemAnimator.ItemHolderInfo postInfo;
