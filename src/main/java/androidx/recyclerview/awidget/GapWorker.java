...
final class GapWorker implements Runnable {
    ...
    ArrayList<ARecyclerView> mRecyclerViews = new ArrayList<>();
    long mPostTimeNs;
    ...
    /**
     * Prefetch information associated with a specific ARecyclerView.
     */
    @SuppressLint("VisibleForTests")
    static class LayoutPrefetchRegistryImpl
            implements ARecyclerView.LayoutManager.LayoutPrefetchRegistry {
        int mPrefetchDx;
        int mPrefetchDy;
        int[] mPrefetchArray;
        ...
        void collectPrefetchPositionsFromView(ARecyclerView view, boolean nested) {
            mCount = 0;
            if (mPrefetchArray != null) {
                Arrays.fill(mPrefetchArray, -1);
            }

            final ARecyclerView.LayoutManager layout = view.mLayout;
    ...
    public void add(ARecyclerView recyclerView) {
        if (ARecyclerView.DEBUG && mRecyclerViews.contains(recyclerView)) {
            throw new IllegalStateException("ARecyclerView already present in worker list!");
        }
        mRecyclerViews.add(recyclerView);
    }

    public void remove(ARecyclerView recyclerView) {
        boolean removeSuccess = mRecyclerViews.remove(recyclerView);
        if (ARecyclerView.DEBUG && !removeSuccess) {
            throw new IllegalStateException("ARecyclerView removal failed!");
        }
    }
     ...
     */
    void postFromTraversal(ARecyclerView recyclerView, int prefetchDx, int prefetchDy) {
    ...
    private void buildTaskList() {
        ...
        for (int i = 0; i < viewCount; i++) {
            ARecyclerView view = mRecyclerViews.get(i);
    ...
    static boolean isPrefetchPositionAttached(ARecyclerView view, int position) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            ARecyclerView.ViewHolder holder = ARecyclerView.getChildViewHolderInt(attachedView);
    ...
    private ARecyclerView.ViewHolder prefetchPositionWithDeadline(ARecyclerView view,
        ...
        ARecyclerView.Recycler recycler = view.mRecycler;
        ARecyclerView.ViewHolder holder;
        try {
    ...
    private void prefetchInnerRecyclerViewWithDeadline(@Nullable ARecyclerView innerView,
                                                       long deadlineNs) {
        if (innerView == null) {
            return;
        }
        ...
        if (innerView.mDataSetHasChangedAfterLayout
            ...
            // ARecyclerView has new data, but old attached views. Clear everything, so that
        ...
        if (innerPrefetchRegistry.mCount != 0) {
            try {
                TraceCompat.beginSection(ARecyclerView.TRACE_NESTED_PREFETCH_TAG);
    ...
    private void flushTaskWithDeadline(Task task, long deadlineNs) {
        long taskDeadlineNs = task.immediate ? ARecyclerView.FOREVER_NS : deadlineNs;
        ARecyclerView.ViewHolder holder = prefetchPositionWithDeadline(task.view,
    ...
    public void run() {
        try {
            TraceCompat.beginSection(ARecyclerView.TRACE_PREFETCH_TAG);
