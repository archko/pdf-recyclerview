...
import androidx.recyclerview.awidget.ARecyclerView.OnItemTouchListener;
import androidx.recyclerview.awidget.ARecyclerView.ViewHolder;

import java.util.ArrayList;
import java.util.List;

/**
 * This is a utility class to add swipe to dismiss and drag & drop support to ARecyclerView.
 * <p>
 * It works with a ARecyclerView and a Callback class, which configures what type of interactions
 * are enabled and also receives events when user performs these actions.
 * <p>
 * Depending on which functionality you support, you should override
 * {@link Callback#onMove(ARecyclerView, ViewHolder, ViewHolder)} and / or
 ...
 * customize these behaviors by overriding {@link Callback#onChildDraw(Canvas, ARecyclerView,
 * ViewHolder, float, float, int, boolean)}
 * or {@link Callback#onChildDrawOver(Canvas, ARecyclerView, ViewHolder, float, float, int,
 * boolean)}.
 * <p/>
 * Most of the time you only need to override <code>onChildDraw</code>.
 */
public class ItemTouchHelper extends ARecyclerView.ItemDecoration
        implements ARecyclerView.OnChildAttachStateChangeListener {
    ...
    /**
     * Horizontal start direction. Resolved to LEFT or RIGHT depending on ARecyclerView's layout
     * direction. Used for swipe & drag control.
     */
    public static final int START = LEFT << 2;

    /**
     * Horizontal end direction. Resolved to LEFT or RIGHT depending on ARecyclerView's layout
     * direction. Used for swipe & drag control.
     */
    ...
    /**
     * Views, whose state should be cleared after they are detached from ARecyclerView.
    ...
    /**
     * Set when ItemTouchHelper is assigned to a ARecyclerView.
     */
     ...
     * {@link Callback#getAbsoluteMovementFlags(ARecyclerView, ViewHolder)} for the current
     * action state.
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
            int mSelectedFlags;
     ...
     */
    private ARecyclerView.ChildDrawingOrderCallback mChildDrawingOrderCallback = null;
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        @Override
        public boolean onInterceptTouchEvent(@NonNull ARecyclerView recyclerView,
        ...
        @Override
        public void onTouchEvent(@NonNull ARecyclerView recyclerView, @NonNull MotionEvent event) {
            mGestureDetector.onTouchEvent(event);
            if (DEBUG) {
                Log.d(TAG,
     ...
     * <p>
     * You can attach ItemTouchHelper to a ARecyclerView via
     * {@link #attachToRecyclerView(ARecyclerView)}. Upon attaching, it will add an item decoration,
     * an onItemTouchListener and a Child attach / detach listener to the ARecyclerView.
     *
    ...
    /**
     * Attaches the ItemTouchHelper to the provided ARecyclerView. If TouchHelper is already
     * attached to a ARecyclerView, it will first detach from the previous one. You can call this
     * method with {@code null} to detach it from the current ARecyclerView.
     *
     * @param recyclerView The ARecyclerView instance to which you want to add this helper or
     *                     {@code null} if you want to remove ItemTouchHelper from the current
     *                     ARecyclerView.
     */
    public void attachToRecyclerView(@Nullable ARecyclerView recyclerView) {
    ...
    @Override
    public void onDrawOver(Canvas c, ARecyclerView parent, ARecyclerView.State state) {
        float dx = 0, dy = 0;
        if (mSelected != null) {
    ...
    @Override
    public void onDraw(Canvas c, ARecyclerView parent, ARecyclerView.State state) {
        ...
        mRecyclerView.post(new Runnable() {
            ...
            public void run() {
                if (mRecyclerView != null && mRecyclerView.isAttachedToWindow()
                        && !anim.mOverridden
                        && anim.mViewHolder.getAdapterPosition() != ARecyclerView.NO_POSITION) {
                    final ARecyclerView.ItemAnimator animator = mRecyclerView.getItemAnimator();
    ...
    boolean scrollIfNecessary() {
        ...
        ARecyclerView.LayoutManager lm = mRecyclerView.getLayoutManager();
        if (mTmpRect == null) {
            mTmpRect = new Rect();
        }
        int scrollX = 0;
    ...
    private List<ViewHolder> findSwapTargets(ViewHolder viewHolder) {
        ...
        final ARecyclerView.LayoutManager lm = mRecyclerView.getLayoutManager();
    ...
    @Override
    public void getItemOffsets(Rect outRect, View view, ARecyclerView parent,
    ...
    void checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        if (mRecyclerView.getScrollState() == ARecyclerView.SCROLL_STATE_DRAGGING) {
            return;
        }
     ...
     * <ul>
     * <li>The provided ViewHolder must be a child of the ARecyclerView to which this
     * ItemTouchHelper
     * is attached.</li>
     ...
     * through ARecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
    ...
    public void startDrag(@NonNull ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            Log.e(TAG, "Start drag has been called with a view holder which is not a child of "
                    + "the ARecyclerView which is controlled by this ItemTouchHelper.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
    ...
    public void startSwipe(@NonNull ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            Log.e(TAG, "Start swipe has been called with a view holder which is not a child of "
                    + "the ARecyclerView controlled by this ItemTouchHelper.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
    ...
    private void addChildDrawingOrderCallback() {
        ...
        if (mChildDrawingOrderCallback == null) {
            mChildDrawingOrderCallback = new ARecyclerView.ChildDrawingOrderCallback() {
                @Override
     ...
     * {@link #getMovementFlags(ARecyclerView, ViewHolder)} and return appropriate set
     ...
     * {@link Callback#onMove(ARecyclerView, ViewHolder, ViewHolder)
     ...
     * in your adapter and also call {@link ARecyclerView.Adapter#notifyItemMoved(int, int)}.
     * To control where a View can be dropped, you can override
     * {@link #canDropOver(ARecyclerView, ViewHolder, ViewHolder)}. When a
    ...
    public abstract static class Callback {
         ...
         * <pre>
         *     public void clearView(ARecyclerView recyclerView, ARecyclerView.ViewHolder viewHolder){
         *         getDefaultUIUtil().clearView(((ItemTouchViewHolder) viewHolder).textView);
         *     }
         *     public void onSelectedChanged(ARecyclerView.ViewHolder viewHolder, int actionState) {
         *         if (viewHolder != null){
         *             getDefaultUIUtil().onSelected(((ItemTouchViewHolder) viewHolder).textView);
         *         }
         *     }
         *     public void onChildDraw(Canvas c, ARecyclerView recyclerView,
         *             ARecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState,
         ...
         *         return true;
         *     }
         *     public void onChildDrawOver(Canvas c, ARecyclerView recyclerView,
         *             ARecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState,
         *             boolean isCurrentlyActive) {
         ...
         * swiping.
         *
         * @param recyclerView The ARecyclerView to which ItemTouchHelper is attached.
         * @param viewHolder   The ViewHolder for which the movement information is necessary.
         * @return flags specifying which movements are allowed on this ViewHolder.
         * @see #makeMovementFlags(int, int)
         * @see #makeFlag(int, int)
         */
        public abstract int getMovementFlags(@NonNull ARecyclerView recyclerView,
         ...
         *
         * @param flags           The flag value that include any number of movement flags.
         * @param layoutDirection The layout direction of the ARecyclerView.
        ...
        final int getAbsoluteMovementFlags(ARecyclerView recyclerView,
                                           ViewHolder viewHolder) {
            final int flags = getMovementFlags(recyclerView, viewHolder);
            return convertToAbsoluteDirection(flags, ViewCompat.getLayoutDirection(recyclerView));
        }

        boolean hasDragFlag(ARecyclerView recyclerView, ViewHolder viewHolder) {
            final int flags = getAbsoluteMovementFlags(recyclerView, viewHolder);
            return (flags & ACTION_MODE_DRAG_MASK) != 0;
        }

        boolean hasSwipeFlag(ARecyclerView recyclerView,
                             ViewHolder viewHolder) {
            final int flags = getAbsoluteMovementFlags(recyclerView, viewHolder);
         ...
         *
         * @param recyclerView The ARecyclerView to which ItemTouchHelper is attached to.
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public boolean canDropOver(@NonNull ARecyclerView recyclerView, @NonNull ViewHolder current,
         ...
         * {@code target}.
         * @see #onMoved(ARecyclerView, ViewHolder, int, ViewHolder, int, int, int)
         */
        public abstract boolean onMove(@NonNull ARecyclerView recyclerView,
         ...
         * The fraction is calculated with respect to ARecyclerView's bounds.
         * <p>
         * Default value is .5f, which means, to swipe a View, user must move the View at least
         * half of ARecyclerView's width or height, depending on the swipe direction.
         *
         ...
         *                    {@link ARecyclerView.ItemDecoration}s.
         ...
         * {@link #getMovementFlags(ARecyclerView, ViewHolder)} method, this method
         ...
         * {@link #clearView(ARecyclerView, ViewHolder)}.
         *
         * @param viewHolder The ViewHolder which has been swiped by the user.
         * @param direction  The direction to which the ViewHolder is swiped. It is one of
         *                   {@link #UP}, {@link #DOWN},
         *                   {@link #LEFT} or {@link #RIGHT}. If your
         *                   {@link #getMovementFlags(ARecyclerView, ViewHolder)}
         *                   method
         ...
         * @see #clearView(ARecyclerView, ViewHolder)
         */
        ...
        private int getMaxDragScroll(ARecyclerView recyclerView) {
        ...
        /**
         * Called when {@link #onMove(ARecyclerView, ViewHolder, ViewHolder)} returns true.
         * <p>
         * ItemTouchHelper does not create an extra Bitmap or View while dragging, instead, it
         * modifies the existing View. Because of this reason, it is important that the View is
         * still part of the layout after it is moved. This may not work as intended when swapped
         * Views are close to ARecyclerView bounds or there are gaps between them (e.g. other Views
         ...
         * <p>
         * Default implementation calls {@link ARecyclerView#scrollToPosition(int)} if the View's
         ...
         *
         * @param recyclerView The ARecyclerView controlled by the ItemTouchHelper.
         ...
         *                     {@link ARecyclerView.ItemDecoration}s.
         */
        public void onMoved(@NonNull final ARecyclerView recyclerView,
                            @NonNull final ViewHolder viewHolder, int fromPos, @NonNull final ViewHolder target,
                            int toPos, int x, int y) {
            final ARecyclerView.LayoutManager layoutManager = recyclerView.getLayoutManager();
        ...
        void onDraw(Canvas c, ARecyclerView parent, ViewHolder selected,
        ...
        void onDrawOver(Canvas c, ARecyclerView parent, ViewHolder selected,
                        List<RecoverAnimation> recoverAnimationList,
                        int actionState, float dX, float dY) {
            final int recoverAnimSize = recoverAnimationList.size();
            for (int i = 0; i < recoverAnimSize; i++) {
         ...
         * {@link #onChildDraw(Canvas, ARecyclerView, ViewHolder, float, float, int,
         * boolean)} or
         * {@link #onChildDrawOver(Canvas, ARecyclerView, ViewHolder, float, float, int, boolean)}.
         *
         * @param recyclerView The ARecyclerView which is controlled by the ItemTouchHelper.
         * @param viewHolder   The View that was interacted by the user.
         */
        public void clearView(@NonNull ARecyclerView recyclerView, @NonNull ViewHolder viewHolder) {
            ItemTouchUIUtilImpl.INSTANCE.clearView(viewHolder.itemView);
        }

        /**
         * Called by ItemTouchHelper on ARecyclerView's onDraw callback.
         * <p>
         ...
         *
         * @param c                 The canvas which ARecyclerView is drawing its children
         * @param recyclerView      The ARecyclerView to which ItemTouchHelper is attached to
         ...
         * @see #onChildDrawOver(Canvas, ARecyclerView, ViewHolder, float, float, int,
         * boolean)
         */
        public void onChildDraw(@NonNull Canvas c, @NonNull ARecyclerView recyclerView,
                                @NonNull ViewHolder viewHolder,
         ...
         * boolean)
         */
        public void onChildDrawOver(@NonNull Canvas c, @NonNull ARecyclerView recyclerView,
                                    ViewHolder viewHolder,
                                    float dX, float dY, int actionState, boolean isCurrentlyActive) {
         ...
         * {@link ARecyclerView.ItemAnimator#getMoveDuration()}, otherwise, it returns
         * {@link ARecyclerView.ItemAnimator#getRemoveDuration()}. If ARecyclerView does not have
         * any {@link ARecyclerView.ItemAnimator} attached, this method returns
         * {@code DEFAULT_DRAG_ANIMATION_DURATION} or {@code DEFAULT_SWIPE_ANIMATION_DURATION}
         * depending on the animation type.
         *
         * @param recyclerView  The ARecyclerView to which the ItemTouchHelper is attached to.
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public long getAnimationDuration(@NonNull ARecyclerView recyclerView, int animationType,
                                         float animateDx, float animateDy) {
            final ARecyclerView.ItemAnimator itemAnimator = recyclerView.getItemAnimator();
         ...
         * <p>
         * You can override this method to decide how much ARecyclerView should scroll in response
         ...
         * bounds, the faster the ARecyclerView will scroll.
         *
         * @param recyclerView        The ARecyclerView instance to which ItemTouchHelper is
         *                            attached to.
         ...
         * @param totalSize           The total size of ARecyclerView in the scroll direction.
         * @param msSinceStartScroll  The time passed since View is kept out of bounds.
         * @return The amount that ARecyclerView should scroll. Keep in mind that this value will
         * be passed to {@link ARecyclerView#scrollBy(int, int)} method.
         */
        @SuppressWarnings("WeakerAccess")
        public int interpolateOutOfBoundsScroll(@NonNull ARecyclerView recyclerView,
     ...
     *         public boolean onMove(ARecyclerView recyclerView,
    ...
    public abstract static class SimpleCallback extends Callback {
         ...
         * {@link #getSwipeDirs(ARecyclerView, ViewHolder)}
         * and / or {@link #getDragDirs(ARecyclerView, ViewHolder)}.
         *
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public int getSwipeDirs(@SuppressWarnings("unused") @NonNull ARecyclerView recyclerView,
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public int getDragDirs(@SuppressWarnings("unused") @NonNull ARecyclerView recyclerView,
                               @SuppressWarnings("unused") @NonNull ViewHolder viewHolder) {
            return mDefaultDragDirs;
        }

        @Override
        public int getMovementFlags(@NonNull ARecyclerView recyclerView,
                                    @NonNull ViewHolder viewHolder) {
